...
- Cadeia principal (rotas da aplicação): stateful (sessão HTTP); o `SecurityContext` é persistido via `SecurityContextRepository`.
- Recursos estáticos: `SecurityFilterChain` dedicada, `STATELESS` e com CSRF desabilitado.
